...
- <For mirrored or cloned open-source repositories: cite `owner/repo @ commit` and repository-relative paths; never paste workstation absolute paths.>
...
- <If local mirrored or cloned open-source repositories were checked, list each as `owner/repo @ commit` plus repository-relative paths. If none were checked, record why external OSS references were not relevant.>
...
- SOW lifecycle: <status/directory checked; if successful close, `Status: completed` and move to `.agents/sow/done/` (local-only, not committed); split/merge/follow-up/regression handling recorded>
