...
sow_dirs=(specs pending current active done)
sow_dir_ok=0
sow_dir_missing=0
for d in "${sow_dirs[@]}"; do
  if [ -d ".agents/sow/$d" ]; then
    ...
    echo "  ${RED}--${NC}  .agents/sow/$d/  (missing; run: mkdir -p .agents/sow/{pending,current,active,done})"
...
if [ -f ".agents/sow/SOW.template.md" ]; then
  ...
  if grep -qF "directory name, not a status value" ".agents/sow/SOW.template.md" 2>/dev/null && grep -qF '`completed` is the successful terminal status' ".agents/sow/SOW.template.md" 2>/dev/null && grep -qF "local-only, not committed" ".agents/sow/SOW.template.md" 2>/dev/null; then
    echo "      ${GREEN}OK${NC}  template includes completed-status and local-only close rule"
    sow_template_completion_rule_missing=0
  else
    echo "      ${RED}--${NC}  template missing completed-status or local-only close rule"
...
for d in pending current done; do
  ...
  while IFS= read -r f; do
    ...
    if grep -Eq '/(home|Users|opt|tmp|var/tmp)/.*/(repos|mirrors)/' "$f" 2>/dev/null; then
      echo "  ${RED}--${NC}  $f  (uses a local mirror absolute path; cite owner/repo @ commit plus repo-relative path)"
...
if $initialized && [ "$sections_missing" -eq 0 ] && [ "$sensitive_warning_missing" -eq 0 ] && [ "$agents_warning_missing" -eq 0 ] && [ "$bridge_missing" -eq 0 ] && [ "$sow_dir_missing" -eq 0 ] && [ "$framework_missing" -eq 0 ] && [ "$sow_status_errors" -eq 0 ] && [ "$pre_impl_errors" -eq 0 ] && [ "$sensitive_gate_errors" -eq 0 ] && [ "$sow_evidence_errors" -eq 0 ] && [ "$todo_untracked_count" -eq 0 ] && [ "$skill_classification_warnings" -eq 0 ]; then
  echo "  ${GREEN}=== SOW initialization complete and clean. ===${NC}"
  exit 0
elif $initialized && [ "$sections_missing" -eq 0 ] && [ "$sensitive_warning_missing" -eq 0 ] && [ "$agents_warning_missing" -eq 0 ] && [ "$bridge_missing" -eq 0 ] && [ "$sow_dir_missing" -eq 0 ] && [ "$framework_missing" -eq 0 ] && [ "$sow_status_errors" -eq 0 ] && [ "$pre_impl_errors" -eq 0 ] && [ "$sensitive_gate_errors" -eq 0 ] && [ "$sow_evidence_errors" -eq 0 ] && [ "$todo_untracked_count" -eq 0 ]; then
  ...
  [ "${sow_template_completion_rule_missing:-0}" -gt 0 ] && echo "    ${YELLOW}- project-local SOW template missing completed-status or local-only close rule${NC}"
  ...
  [ "$mirror_path_violations" -gt 0 ] && echo "    ${YELLOW}- ${mirror_path_violations} SOW file(s) use local mirror absolute paths instead of owner/repo @ commit citations${NC}"
